...
def makeImageRGB(rows, cols, mode="RGB"):
    image = Image.new(mode, (cols, rows), (0,0,0))        # cols=>width,rows=>height
    pixels = image.load()
    return image


# converts the given matrix in RGB format, i.e. each pixel (R,G,B)
# to GRAY format, i.e. each pixel a single value
def rgb2gray( rgb ):
...
# converts the given matrix in RGB format, i.e. each pixel (R,G,B)
# to LAB format, i.e. each pixel (L, a, b)
...
def mat2rgbimg( M, mode="RGB" ):
    rows, cols = getSize( M )
    image = makeImageRGB( rows, cols, mode )
    pixels = image.load()
    for r in range(0, rows):
        for c in range(0, cols):
...
# converts the given PIL image to matrix
def rgbimg2mat( image ):
    width, height = image.size
    rows, cols = height, width
    M = makeVecMatrix( rows, cols, 3 )
    pixels = image.load()
    for r in range(0, rows):
        for c in range(0, cols):
            M[r, c] = pixels[c, r]             # (r,c)=(y,x)=>(x, y)
...
# returns the 3 (eVal,eVec) pairs for the given mXnX3 matrix
# the largest eigenvalue/vector pair is first
def pca( M ):
    # convert the mXnX3 matrix into a list of the 3D vectors
    rows, cols = getSize(M)
    M = M.reshape( rows*cols, 3 )
    
    # shift the data by the mean of each component
    M = M - M.mean( axis=0 )

    # compute covariance matrix
    covM = numpy.cov([M[0,:],M[1,:],M[2,:]])

    # compute eigen values and vectors
    evalM, evecM = numpy.linalg.eig(covM)

    # return the eigen value/vector pairs (highest eigenval first)
    return sorted( zip(evalM, evecM), reverse=True )
