...
s_factor = 3      # smoothing factor, number of times to loop to get T'x, T'y
...
def lineDrawing(image):
    ...
    print(Tx[100:120, 200:210])
    print(Ty[100:120, 200:210])
    Tx_p, Ty_p = computeETF(Tx, Ty, G)
    print(Tx_p[100:120, 200:210])
    print("Saving...")
...
def gradientTangents(Gx, Gy):
    ...
    Tx, Ty = support.makeMatrix(rows,cols), support.makeMatrix(rows,cols)
    for r in range(0, rows, 1):
        for c in range(0, cols, 1):
            vec = support.makeVector(Gx[r,c],Gy[r,c])
            tangent = rotateCCW(vec)
            #print(tangent)
            normaltangent = normalizeVector(support.makeVector(tangent[0], tangent[1]))
            #print(normaltangent)
            ...
            for c in range(0, cols, 1):
                ...
                for rn in range(rn_init, rn_end, 1):
                    for cn in range(cn_init, cn_end, 1):
                        ...
                        if sWeight(x,y) == 1:
                            mW = mWeight(x,y,G)
                            dW = dWeight(x,y,Tx_cur, Ty_cur)
                            phi_eq = phi(x,y,Tx_cur, Ty_cur)
                            #print(x, y)
                            sum_x = sum_x + phi_eq*Tx_cur[y[0],y[1]]*mW*dW
                            sum_y = sum_y + phi_eq*Ty_cur[y[0],y[1]]*mW*dW

                #Normalize Vector
                v = support.makeVector(sum_x, sum_y)
                #print("v", v)
                normal_v = normalizeVector(v)
                #print("normal",normal_v)
